...
CREATE TABLE IF NOT EXISTS vehicle (
    ...
    license_number  VARCHAR(13)  REFERENCES driver(license_number) ON DELETE CASCADE
);

-- 3. Vehicle Registration Table
CREATE TABLE IF NOT EXISTS vehicle_registration (
    registration_number  VARCHAR(20)  PRIMARY KEY,
    plate_number         VARCHAR(7)  NOT NULL REFERENCES vehicle(plate_number) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS violation (
    ...
    license_number            VARCHAR(13)  REFERENCES driver(license_number) ON DELETE CASCADE,
    plate_number              VARCHAR(7)  REFERENCES vehicle(plate_number) ON DELETE CASCADE
);
